...
// 更新成员榜
update_vote_member_rank($pj_id, 66);
*/
...
// 投票券
$sql = "SELECT * FROM ticket_count WHERE pj_id = {$pj_id} ORDER BY sort_no";
$db->query($sql);
$rows = $db->fetchAll();
...
// 电子券
$sql = "SELECT * FROM e_ticket_count WHERE pj_id = {$pj_id} ORDER BY sort_no";
$db->query($sql);
$rows = $db->fetchAll();
...
foreach ($rows as $row)
{
  ...
  $sell_count = intval($row['sell_count']);
  $sell_sum = intval($row['sell_sum']);
  $e_ticket_str .= "<dt>$e_type</<dt><dd>单价：$e_amount 元 销售次数：$sell_count 销售数量：$sell_sum</dd>";
}
$e_ticket_str .= "</dl>";
