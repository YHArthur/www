...
if($week_rows ==''){
  ...
  $id_count = serch_week_rpt_detail_id($url_key,$week_time,$endLastweek);
  ...
  $data['rpt_type'] = 'week';
  $data['rpt_time'] = date('Y-m-s h:i:s',time()/1000);
  $creat = creat_rpt_detail($data);
  array_splice($data, 0, count($data));
  $week_rows = get_week_overview($url_key);
}
...
while($today){
  //获取每日浏览数
  $rows = get_day_overview($url_key, $today,$endday);
  if(count($rows)){
    foreach($rows as $row){
